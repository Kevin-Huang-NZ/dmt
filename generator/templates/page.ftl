<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.[=firstFieldName]" placeholder="" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
      <el-input v-model="listQuery.[=secondFieldName]" placeholder="" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column label="[=field.fieldName]" align="center">
        <template slot-scope="{row}">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="120px" style="margin-left:50px;margin-right:50px;">
        <#list fields as field>
        <el-form-item label="[=field.fieldName]" prop="[=field.fieldName]">
          <el-input v-model="temp.[=field.fieldName]" />
        </el-form-item>
        </#list>
...
<script>
import { list[=beanName], get[=beanName], create[=beanName], update[=beanName], delete[=beanName] } from '@/api/sys'
...
export default {
    ...
    return {
      ...
      rules: {
        ...
        [=field.fieldName]: [
          ...
          { required: true, message: '请填写。', trigger: 'blur' }<#if field.maxLen?has_content >,</#if>
          </#if>
          <#if field.maxLen?has_content >
          { max: [=field.maxLen], message: '长度不能超过[=field.maxLen]字符。', trigger: 'blur' }
          </#if>
        ]<#sep>,</#sep>
        </#list>
  ...
  methods: {
    getList() {
      ...
      list[=beanName](this.listQuery).then(response => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          create[=beanName](this.temp).then(() => {
    ...
    handleUpdate(row) {
      const { id } = row
      get[=beanName](id).then(response => {
        // this.temp = Object.assign({}, response) // copy obj
        this.temp = ((r) => ({
          id: r.id,
          <#list fields as field>
          [=field.fieldName]: r.[=field.fieldName]<#sep>,</#sep>
          </#list>
        }))(response)
