<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.roleNo" placeholder="" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column label="roleNo" align="center">
        <template slot-scope="{row}">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="120px" style="margin-left:50px;margin-right:50px;">
        <el-form-item label="roleNo" prop="roleNo">
          <el-input v-model="temp.roleNo" />
        </el-form-item>
...
<script>
import { listSysRoleFun, getSysRoleFun, createSysRoleFun, updateSysRoleFun, deleteSysRoleFun } from '@/api/sys'
...
export default {
  ...
  methods: {
    getList() {
      ...
      listSysRoleFun(this.listQuery).then(response => {
    ...
    //   updateSysRoleFun(tempData).then(() => {
    //     this.$message({
    //       message: '更新成功',
    //       type: 'success',
    //       duration: 1000
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createSysRoleFun(this.temp).then(() => {
    ...
    handleUpdate(row) {
      const { id } = row
      getSysRoleFun(id).then(response => {
