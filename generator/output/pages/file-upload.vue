<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.tableName" placeholder="" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
      <el-input v-model="listQuery.rowId" placeholder="" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column label="rowId" align="center">
        <template slot-scope="{row}">
          <span>{{ row.rowId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="fullPath" align="center">
        <template slot-scope="{row}">
      ...
      <el-table-column label="deleteStatus" align="center">
        <template slot-scope="{row}">
          <span>{{ row.deleteStatus }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="120px" style="margin-left:50px;margin-right:50px;">
        ...
        <el-form-item label="rowId" prop="rowId">
          <el-input v-model="temp.rowId" />
        </el-form-item>
        <el-form-item label="fullPath" prop="fullPath">
          <el-input v-model="temp.fullPath" />
        </el-form-item>
        <el-form-item label="fileName" prop="fileName">
          <el-input v-model="temp.fileName" />
        </el-form-item>
        <el-form-item label="deleteStatus" prop="deleteStatus">
          <el-input v-model="temp.deleteStatus" />
        </el-form-item>
...
<script>
import { listFileUpload, getFileUpload, createFileUpload, updateFileUpload, deleteFileUpload } from '@/api/sys'
...
export default {
  ...
  methods: {
    getList() {
      ...
      listFileUpload(this.listQuery).then(response => {
    ...
    //   updateFileUpload(tempData).then(() => {
    //     this.$message({
    //       message: '更新成功',
    //       type: 'success',
    //       duration: 1000
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createFileUpload(this.temp).then(() => {
    ...
    handleUpdate(row) {
      const { id } = row
      getFileUpload(id).then(response => {
