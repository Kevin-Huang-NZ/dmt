...
public class CountryController extends BaseController {
	...
	private void checkCreate(Country country) throws BusinessException {
		...
		if (isEmpty(country.getCountryCode())) {
			throw new BusinessException(EmBusinessError.PARAMETER_VALIDATION_ERROR,"国家代码不能为空。");
		}
		if (overLength(country.getCountryCode(), 20)) {
			throw new BusinessException(EmBusinessError.PARAMETER_VALIDATION_ERROR,"国家代码超长，最多20个字符。");
		}
		if (isEmpty(country.getCountryName())) {
			throw new BusinessException(EmBusinessError.PARAMETER_VALIDATION_ERROR,"国家名称不能为空。");
		}
		if (overLength(country.getCountryName(), 100)) {
			throw new BusinessException(EmBusinessError.PARAMETER_VALIDATION_ERROR,"国家名称超长，最多100个字符。");
		}
	}
