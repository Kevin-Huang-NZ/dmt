  ...
  <div class="app-container">
    <div class="filter-container">
      <el-alert type="info" :closable="false">
        <el-tag>{{ currentCountry.countryName }}</el-tag> - <el-tag>{{ currentLanguage.languageName }}</el-tag> 的常用词
      </el-alert>
    </div>
    <div class="filter-container">
      ...
      <el-select v-model="listQuery.originalType" class="filter-item" clearable filterable placeholder="全部类型" @change="handleFilter">
        <el-option v-for="item in originalTypeOptions" :key="item.key" :label="item.display_name" :value="item.key" />
      </el-select>
    ...
    >
      ...
      <el-table-column label="原文" align="center" prop="t_original" sortable="custom" :class-name="getSortClass('t_original')">
        <template slot-scope="{row}">
          <span>{{ row.original }}</span>
        </template>
      </el-table-column>
      <el-table-column label="原文缩写" align="center">
        <template slot-scope="{row}">
          <span>{{ row.originalAbbr }}</span>
        </template>
      </el-table-column>
      <el-table-column label="原文类型" align="center">
        <template slot-scope="{row}">
          <span>{{ row.originalType | originalTypeFilter }}</span>
        </template>
      </el-table-column>
      <el-table-column label="罗马字母转写" align="center" prop="t_roman" sortable="custom" :class-name="getSortClass('t_roman')">
        <template slot-scope="{row}">
          <span>{{ row.roman }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="120px" style="margin-left:50px;margin-right:50px;">
        ...
        <el-form-item label="原文缩写" prop="originalAbbr">
          <el-input v-model="temp.originalAbbr" />
        </el-form-item>
        <el-form-item label="原文类型" prop="originalType">
          <el-select v-model="temp.originalType" class="filter-item" placeholder="请选择" clearable>
            <el-option v-for="item in originalTypeOptions" :key="item.key" :label="item.display_name" :value="item.key" />
          </el-select>
        </el-form-item>
...
<script>
import { listCtCommon, getCtCommon, createCtCommon, updateCtCommon, deleteCtCommon, importCtCommon, clearCtCommon } from '@/api/ct'
import { getCountryByCode, getLanguageByCode } from '@/api/sys'
import waves from '@/directive/waves' // waves directive
// import { parseTime } from '@/utils'
import Pagination from '@/components/Pagination' // secondary package based on el-pagination
// import { validPhone } from '@/utils/validate'
import { originalTypeOptions, originalTypeKeyValue, matchWayOptions, matchWayKeyValue } from '@/utils/options'
...
export default {
  ...
  directives: { waves },
  filters: {
    originalTypeFilter(originalType) {
      return originalTypeKeyValue[originalType]
    },
    ...
    return {
      ...
      rules: {
        original: [
          ...
          { max: 200, message: '原文长度不能超过200字符。', trigger: 'blur' }
        ],
        originalAbbr: [
          { max: 100, message: '原文缩写长度不能超过100字符。', trigger: 'blur' }
        ],
        originalType: [
          { required: true, message: '请选择原文类型。', trigger: 'blur' }
        ],
        roman: [
          { max: 200, message: '罗马字母转写长度不能超过200字符。', trigger: 'blur' }
        ],
        matchWay: [
          { required: true, message: '请选择匹配方式。', trigger: 'blur' }
        ],
        matchParams: [
          { max: 200, message: '匹配参数长度不能超过200字符。', trigger: 'blur' }
        ],
        transliteration: [
          { max: 200, message: '音译长度不能超过200字符。', trigger: 'blur' }
        ],
        freeTranslation: [
          { max: 200, message: '意译长度不能超过200字符。', trigger: 'blur' }
        ]
  ...
  methods: {
    getList() {
      ...
      listCtCommon(this.listQuery).then(response => {
    ...
    handleDownload() {
      ...
      listCtCommon(tempData).then(response => {
        const { dataList } = response
        this.allList = dataList
        import('@/vendor/Export2Excel').then(excel => {
          const tHeader = ['原文', '原文缩写',
            { v: '原文类型', c: '原文类型：1-人名；2-通名；3-形容词；x-其它' },
            { v: '原文类型-含义', c: '导入数据时，系统读取的是【原文类型】列，请确保填写了正确的值' },
            '罗马字母转写',
            { v: '匹配方式', c: '匹配方式：1-精确；2-前缀（词头）；3-后缀（词尾）；4-前置（在xxx之前）；5-后置（在xxx之后）' },
            { v: '匹配方式-含义', c: '导入数据时，系统读取的是【匹配方式】列，请确保填写了正确的值' },
            '匹配参数', '音译', '意译']
          const filterVal = ['original', 'originalAbbr', 'originalType', 'originalTypeName', 'roman', 'matchWay', 'matchWayName', 'matchParams', 'transliteration', 'freeTranslation']
    ...
    formatJson(filterVal) {
      return this.allList.map(v => filterVal.map(j => {
        if (j === 'originalTypeName') {
          return originalTypeKeyValue[v['originalType']]
        } else if (j === 'matchWayName') {
    ...
    importExcel(res) {
      ...
      importCtCommon(fd).then((response) => {
        this.$message({
          message: '导入成功。',
          type: 'success',
          duration: 1000
