  ...
  <div class="app-container">
    <div class="filter-container">
      <el-alert type="info" :closable="false">
        <el-tag>{{ currentCountry.countryName }}</el-tag> - <el-tag>{{ currentLanguage.languageName }}</el-tag> 的罗马字母转换表
      </el-alert>
    </div>
    <div class="filter-container">
    ...
    >
      ...
      <el-table-column label="原文字母" align="center" prop="t_original_alpha" sortable="custom" :class-name="getSortClass('t_original_alpha')">
        <template slot-scope="{row}">
          <span>{{ row.originalAlpha }}</span>
        </template>
      </el-table-column>
      <el-table-column label="罗马字母" align="center" prop="t_roman_alpha" sortable="custom" :class-name="getSortClass('t_roman_alpha')">
        <template slot-scope="{row}">
          <span>{{ row.romanAlpha }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="120px" style="margin-left:50px;margin-right:50px;">
        <el-form-item label="原文字母" prop="originalAlpha">
          <el-input v-model="temp.originalAlpha" />
        </el-form-item>
        <el-form-item label="罗马字母" prop="romanAlpha">
          <el-input v-model="temp.romanAlpha" />
        </el-form-item>
...
<script>
import { listCtRoman, getCtRoman, createCtRoman, updateCtRoman, deleteCtRoman, importCtRoman, clearCtRoman } from '@/api/ct'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        originalAlpha: [
          { required: true, message: '请填写原文字母。', trigger: 'blur' },
          { max: 10, message: '原文字母长度不能超过10字符。', trigger: 'blur' }
        ],
        romanAlpha: [
          { required: true, message: '请填写罗马字母。', trigger: 'blur' },
          { max: 10, message: '罗马字母长度不能超过10字符。', trigger: 'blur' }
        ]
  ...
  methods: {
    getList() {
      ...
      listCtRoman(this.listQuery).then(response => {
    ...
    handleUpdate(row) {
      const { id } = row
      getCtRoman(id).then(response => {
        // this.temp = Object.assign({}, response) // copy obj
        this.temp = ((r) => ({
          id: r.id,
          countryCode: r.countryCode,
          languageCode: r.languageCode,
          originalAlpha: r.originalAlpha,
          romanAlpha: r.romanAlpha
        }))(response)
    ...
    handleDownload() {
      ...
      listCtRoman(tempData).then(response => {
        const { dataList } = response
        this.allList = dataList
        import('@/vendor/Export2Excel').then(excel => {
          const tHeader = ['原文字母', '罗马字母']
          const filterVal = ['originalAlpha', 'romanAlpha']
    ...
    importExcel(res) {
      ...
      importCtRoman(fd).then((response) => {
        this.$message({
          message: '导入成功。',
          type: 'success',
          duration: 1000
