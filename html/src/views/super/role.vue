<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.roleNo" placeholder="角色编号" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column label="角色编号" prop="t_role_no" sortable="custom" align="center" width="160px" :class-name="getSortClass('t_role_no')">
        <template slot-scope="{row}">
      ...
      <el-table-column label="操作" align="center" width="330px" class-name="small-padding fixed-width">
        <template slot-scope="{row,$index}">
          ...
          <router-link :to="'/super/choose-fun/'+row.roleNo" style="margin-left: 10px">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="120px" style="margin-left:50px;margin-right:50px;">
        <el-form-item label="角色编号" prop="roleNo">
          <el-input v-model="temp.roleNo" :readonly="dialogStatus!=='create'" />
        </el-form-item>
...
<script>
import { listSysRole, getSysRole, createSysRole, updateSysRole, deleteSysRole } from '@/api/super'
  ...
  data() {
    ...
    const validateRoleNo = (rule, value, callback) => {
      if (!/^[a-zA-Z0-9]{1}$/.test(value)) {
        callback(new Error('请填写正确格式的角色编号,角色编号为1位字母或数字。'))
      } else {
        callback()
      }
    }
    return {
      ...
      rules: {
        roleNo: [{ required: true, trigger: 'blur', validator: validateRoleNo }],
        roleName: [
          { required: true, message: '请填写角色名称。', trigger: 'blur' },
          { max: 20, message: '角色名称长度不能超过20字符。', trigger: 'blur' }
        ],
        memo: [
