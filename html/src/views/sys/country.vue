<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.countryCode" placeholder="国家代码" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
      <el-input v-model="listQuery.countryName" placeholder="国家名称" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column label="国家代码" align="center" prop="t_country_code" sortable="custom" :class-name="getSortClass('t_country_code')">
        <template slot-scope="{row}">
          <span>{{ row.countryCode }}</span>
        </template>
      </el-table-column>
      <el-table-column label="国家名称" align="center" prop="t_country_name" sortable="custom" :class-name="getSortClass('t_country_name')">
        <template slot-scope="{row}">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="120px" style="margin-left:50px;margin-right:50px;">
        <el-form-item label="国家代码" prop="countryCode">
          <el-input v-model="temp.countryCode" :readonly="dialogStatus!=='create'" />
        </el-form-item>
...
<script>
import { listCountry, getCountry, createCountry, updateCountry, deleteCountry } from '@/api/sys'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        countryCode: [
          { required: true, message: '请填写国家代码。', trigger: 'blur' },
  ...
  methods: {
    getList() {
      ...
      listCountry(this.listQuery).then(response => {
